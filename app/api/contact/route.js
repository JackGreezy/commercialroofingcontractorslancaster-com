...
const SITE = {
  name: "Commercial Roofing Contractors of Lancaster",
  domain: "commercialroofingcontractorslancaster.com",
  email: "hello@commercialroofingcontractorslancaster.com",
  phone: "555-555-6148"
};
...
function allowed(request) {
  const key = clean(request.headers.get("cf-connecting-ip")) || clean(request.headers.get("x-forwarded-for")).split(",")[0] || "unknown";
  const now = Date.now();
  const state = buckets.get(key) || { count: 0, reset: now + WINDOW_MS };
  if (state.reset <= now) {
    state.count = 0;
...
async function payload(request) {
  ...
  if (type.includes("application/x-www-form-urlencoded")) return Object.fromEntries(new URLSearchParams(await request.text()).entries());
...
function normalize(body) {
  ...
  return {
    ...
    service: clean(body.roofingNeed || body.service || body.inquiryType || "Commercial Roofing"),
...
async function sendEmail({ to, subject, html, replyTo }) {
  ...
  const response = await fetch("https://api.sendgrid.com/v3/mail/send", {
    ...
    body: JSON.stringify({
      ...
      categories: ["contact-form", "commercialroofingcontractorslancaster-com"]
    })
  });
...
function leadHtml(lead) {
  const rows = [
    ...
    ["Roofing need", lead.service], ["Property address", lead.address],
    ["Project details", lead.details], ["Submitted", lead.submittedAt]
  ];
...
export async function POST(request) {
  if (!allowed(request)) return respond(request, { ok: false, error: "Too many requests. Please try again shortly." }, 429);
  let body;
  try { body = await payload(request); } catch { return respond(request, { ok: false, error: "Invalid request." }, 400); }
  if (clean(body.website) || clean(body._company) || clean(body.honeypot)) return respond(request, { ok: true });
  const lead = normalize(body);
  const error = validate(lead);
  if (error) return respond(request, { ok: false, error }, 400);
  try {
    await Promise.all([
      sendEmail({ to: recipients(), subject: `New ${lead.service} inquiry from ${lead.name}`, html: leadHtml(lead), replyTo: lead.email }),
      sendEmail({
        to: [lead.email],
